...
def export_telemetry_comparison(year, location, session_type, driver1, driver2):
    print(f"--- {year} {location} [{session_type}] {driver1} vs {driver2} 分析中... ---")
    ...
    try:
        ...
        # 5. 各種グラフの描画
        ax[0].plot(d1_tel['Distance'], d1_tel['Speed'], color=color1, label=f"{driver1}")
        ax[0].plot(d2_tel['Distance'], d2_tel['Speed'], color=color2, label=f"{driver2}")
        ...
        save_path = f"output/{year}_{location}_{session_type}_{driver1}_{driver2}.png"
